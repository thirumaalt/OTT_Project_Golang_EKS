...
def transcode_quality(input_file: str, output_dir: str, quality: dict, use_gpu: bool):
...
def create_master_playlist(output_dir: str, qualities: list):
...
def upload_hls_to_s3(local_output_dir: str, s3_key_prefix: str):
...
def transcode_to_hls(input_path: str, output_dir: str, s3_key_prefix: str = None):
    ...
    For local storage: Downloads from S3 (if STORAGE_TYPE=s3), transcodes locally, uploads back to S3.
    For local storage only: Works directly on filesystem.
